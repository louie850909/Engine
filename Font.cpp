...
Font::Font(ID3D11Device* device, const char* filename, int maxSpriteCount)
{
	...
	// .fnt 読み込み
	{
		...
		for (int i = 0; i < pages; i++)
		{
			...
			HRESULT hr = load_texture_from_file(device, wfname, shaderResourceViews.at(i).GetAddressOf(), &texture2d_desc);
...
Font::~Font()
{
	vertexShader.Get()->Release();
	pixelShader.Get()->Release();
	inputLayout.Get()->Release();
	vertexBuffer.Get()->Release();
	indexBuffer.Get()->Release();
	blendState.Get()->Release();
	rasterizerState.Get()->Release();
	depthStencilState.Get()->Release();
